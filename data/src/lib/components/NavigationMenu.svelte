...
>
				...
				<div class="pl-2 flex items-center gap-1">
					<div class="inline-grid *:[grid-area:1/1]">
						<div
							...
							class:status-success={websocketState.connected}
							class:status-error={!websocketState.connected}
							class:status-warning={websocketState.state ==
								"reconnecting"}
						></div>
						<div
							class="status"
					...
					<span
						class="text-xs"
						class:text-success={websocketState.connected}
						class:text-error={!websocketState.connected}
	...
	<div class="divider hidden lg:flex m-1"></div>
	<!-- Menu Section -->
	...
	<div class="flex self-start justify-between p-4">
		<label class="swap swap-rotate" aria-label="Toggle theme">
			<input
				type="checkbox"
				onclick={toggleTheme}
				checked={getCurrentTheme() !== "dracula"}
			/>
			<svg
